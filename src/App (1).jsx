...
function FleetPage({ vehicles, setVehicles, apiKey, usage, setUsage, livrePolice, setLivrePolice, viewMode, garageId }) {
  ...
  const save = (v) => {
    ...
    if (!exists && setLivrePolice) {
      console.log("🚗 Nouveau véhicule détecté, création LP...", v.marque, v.modele, v.plate);
      // Utiliser la forme fonctionnelle pour avoir le state le plus récent
      setLivrePolice(currentLP => {
        const lp = currentLP || [];
        console.log("📋 LP actuel:", lp.length, "entrées", lp.map(e => e.immat));
        const alreadyInLP = lp.find(e => 
          (v.plate && e.immat && e.immat === v.plate) || 
          (v.id && e.vehicle_id && e.vehicle_id === v.id)
        );
        if (alreadyInLP) {
          console.log("⚠️ Déjà dans le LP, skip");
          return lp;
        }
        console.log("✅ Ajout au LP, nouvelle entrée #" + (lp.length + 1));
